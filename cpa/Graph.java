...
public class Graph implements GraphInterface {
  ...
  private QueueInterface<GraphNode> topologicalSort() {
    Queue<GraphNode> tempQueue = new Queue<>(), resultQueue = new Queue<>();
    setIncomingDegree();
    tempQueue.enqueue(start);
    while (!tempQueue.isEmpty()) {
      GraphNode top = tempQueue.dequeue();
      resultQueue.enqueue(top);
      GenericListInterface<GraphEdge> edges = top.getOutgoingEdges();
      for (GraphEdge edge : edges) {
  ...
  private void computeEarliestCompletionTime() {
    ...
    GenericListInterface<GraphEdge> edges = start.getOutgoingEdges();
    for (GraphEdge edge : edges) {
      GraphNode node = edge.getEndPoint();
      if (node.EarliestCompletionTime() < start.EarliestCompletionTime() + edge.getTaskDuration()) {
        node.setEarliestCompletionTime(start.EarliestCompletionTime() + edge.getTaskDuration());
      }
    }
    while (!sortedNodes.isEmpty()) {
      GraphNode current = sortedNodes.dequeue();
      edges = current.getOutgoingEdges();
      for (GraphEdge edge : edges) {
        GraphNode node = edge.getEndPoint();
        if (node.EarliestCompletionTime()
            < current.EarliestCompletionTime() + edge.getTaskDuration()) {
          node.setEarliestCompletionTime(current.EarliestCompletionTime() + edge.getTaskDuration());
        }
      }
    }
  }
  ...
  private void parseGraph(String inputFile) throws FileNotFoundException {
      ...
      if (line[0].equals("Finish")) {
        ...
        if (line.length == 2) {
          ...
          for (int j = 1; j < line.length; j++) {
            ...
            if (dependentTask.getEndPoint() != null) {
              dummyTask.setStartPoint(dependentTask.getEndPoint());
              dependentTask.getEndPoint().addOutgoingEdge(dummyTask);
            } else {
              GraphNode intermediateNode = new GraphNode();
              dependentTask.setEndPoint(intermediateNode);
              intermediateNode.addIncomingEdge(dependentTask);
